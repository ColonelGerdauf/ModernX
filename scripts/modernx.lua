...
-- tweaked by Colonel_Gerdauf
-- https://github.com/ColonelGerdauf/ModernX/
...
-- you can change them in osc.conf
local user_opts = {
...
function show_message(text, duration)
    ...
    -- cuts the text short, otherwise the following
...
function osc_init()
    ...
    ne.slider.tooltipF = function (pos)
        local duration = mp.get_property_number('duration', nil) if not ((duration == nil) or (pos == nil)) then
    ...
    ne.eventresponder['wheel_up_press'] =
        function (element) set_volume(state.proc_volume+5) end
